...
def PredictScore(train_drug_mic_matrix, drug_matrix, mic_matrix, seed, sizes):
    ...
    model = MKGCN.Model(sizes, drug_matrix, mic_matrix,drug_mic_matrix)
    print(model)
...
def cross_validation_experiment(drug_mic_matrix, drug_matrix, mic_matrix, sizes):
    ...
    for k in range(sizes.k_fold):
        ...
        print(metric_tmp)
        with open('C:\新建文件夹\MKGCN-main\code\myfile.txt', 'a') as file:
          for item in metric_tmp:
            file.write(str(item) + '\n')
...
if __name__ == "__main__":
    ...
    #matDT = interaction = np.loadtxt("C:\新建文件夹\MKGCN-main\data\MicrobeDrugA\\aBiofilm\\adj_matrix.txt", delimiter='\t')
    ...
    np.savetxt('.\pre_matrix.txt', pre_matrix)
